...
class Board extends Component {
    ...
    restartBoard = () =>{
        ...
        fetch("https://us-central1-skooldio-courses.cloudfunctions.net/react_01/random").then((res) => {
                return res.json();
            }).then((res) =>{
                this.setState({
                    board : res.board,
                    timer : 0 ,
                    initial : res.board.map(row => row.map(item => item !== 0 ) ),
                    loading : false
                })
            })
    }
    ...
    render(){
        return(
        <div>
            <p className='timer'>Elapse Time: {this.state.timer} Second</p>
            <div className="board">
                {!this.state.loading && 
                    this.state.board.map((row, i) => 
                        row.map((number, j) => (
                            <Cell key={`cell-${i}-${j}`} isInitial={this.state.initial[i][j]} number={number} onChange={(newNumber)=>{
